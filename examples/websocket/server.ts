...
const io = new Server(httpServer, {
  ...
  path: '/',
  cors: {
    // Restrict to the allowed origin (env-configurable). Wildcard CORS lets
    // any website connect to this socket (06 section 5: reduce attack surface).
...
// Use crypto.randomUUID for unpredictable, collision-resistant ids.
// Math.random() is predictable and a bad pattern to model.
const generateMessageId = () => crypto.randomUUID().slice(0, 9)
